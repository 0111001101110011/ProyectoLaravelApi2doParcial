...
<div class="container-fluid">
    ...
    <div class="card shadow">
        <div class="card-header py-3"><a class="btn btn-primary" role="button" href="{{route('armar.create')}}" ><i class="fa fa-plus"></i>&nbsp; &nbsp;Agregar Registro</a></div>
        <div class="card-body">
            <form action="/search">
                <div class="row">
                    <div class="col-md-4">
            ...
            <div class="table-responsive table mt-2" id="dataTable" role="grid" aria-describedby="dataTable_info">
                <table class="table dataTable my-0" id="dataTable">
                    ...
                    <tbody>
                        @foreach($armar as $arma)
                        <tr>
                            <td class="text-center">{{ $arma -> id }}</td>
                            <td class="text-center">{{ $arma -> id_user }}</td>
                            <td class="text-center">{{ $arma -> estado }}</td>
                            <td class="text-center">{{ $arma -> fecha_hora }}</td>
                            <td class="text-center">{{ $arma -> ubicacion }}</td>
                            <td>
                                <div class="btn-group" role="group" style="filter: brightness(100%);">
                                    <a class="btn btn-primary" type="button" href="{{ route('armar.show',$arma -> id)}}" style="background-color: #4e73df;">  <i class="fa fa-eye"style="color: rgb(255,255,255);" ></i></a>
                                    <a class="btn btn-primary" type="button" href="{{ route('armar.edit',$arma -> id)}}" style="background-color: #808080;">  <i class="fa fa-pencil" style="color: rgb(255,255,255);"></i></a>
                                   
                                    @csrf
                                    @method('DELETE')

                                    <a href="javascript:;" data-toggle="modal" onclick="deleteData({{$arma->id}})" data-target="#DeleteModal" class="btn btn-danger">
...
<script type="text/javascript">
    ...
    {
        var id = id;
        var url = '{{ route("armar.destroy", ":id") }}';
